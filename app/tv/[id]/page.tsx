...
import TvCast from '@/Components/Slide/TvCast'
import SimilarTvShow from '@/Components/Slide/SimilarMovie'
import Icon021VideoCamera from './Icon021VideoCamera'
import { getVideoTv } from "@/Components/Server/ServerComponent";
...
const MovieDetails = () => {
    ...
    const[tv, setTv] = useState<any>(null); // Use any or define a proper type
    ...
    const handleTrailer = async() => {
        ...
        const videoData = await getVideoTv(Number(id))
        setloader(true)
    ...
    useEffect(() => {
        if (id) {
            fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=d92de49cc045b7079ff263d81acb9db2`)
    ...
    const ShowReadMore = tv.overview.length > 400;


    return (
       <>
       
       <div className="w-full h-screen Background"  style={{backgroundImage: `linear-gradient(to right, #000, transparent), url(${tv.backdrop_path ? `${OriginalBackground}/${tv.backdrop_path}` : "https://www.katebackdrop.com/cdn/shop/products/Kate_Movie_Night_Backdrop_Red_Curtain_for_Photography_10x8ft_BH1030936U.jpg?v=1631776029&width=1000"})` ,
    ...
    <div className="w-auto h-auto Alldetails">
        ...
        <div className="ml-20 details flex">
            ...
            <div className="ImagetoDate">
                <img
                    src={tv.poster_path ? `https://image.tmdb.org/t/p/w500/${tv.poster_path}` : "https://img.freepik.com/premium-vector/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available_87543-11093.jpg"}
                    className={`${tv.poster_path ? 'h-96 w-72 PosterImg Poster500' : 'w-72 h-96 PosterImg Poster500'}`}
                />
                ...
                <div className="Holder">
                    ...
                    <div className="TitleToDate">
                        ...
                        <p className="text-white font-bold text-2xl Title">{tv.name}</p>
                        <div className="flex items-center mt-1 MovieVoteCount">
                            <IconBxLike />
                            <p className="text-white ml-2 MovieData">{tv.vote_count}</p>
                        </div>
                        <div className="flex items-center mt-1">
                            <IconStarFill />
                            <p className="text-white ml-2 MovieData">{tv.vote_average.toString().substring(0, 3)}</p>
                        </div>
                        <div className="flex items-center mt-1">
                            <Icon021VideoCamera />
                            <p className="text-white ml-1 MovieData">{formatDate(tv.release_date)}</p>
                        </div>
                        <div className="flex items-center mt-1">
                            <p className="text-white ml-1 MovieData"> <span className="font-bold">Genre{tv.genres.length !== 1 ? 's' : ''}:</span> {getGenreNames(tv.genres)}</p>   
                        </div>
            ...
            <div className={`${see ? 'flex-col ml-5 w-1/2 microDetails' : 'flex-col ml-5 w-96 microDetails'}`}>
                <p className="text-white mt-5 font-bold text-xl MovieTitleSynopsis">{tv.title} Synopsis:</p>
                <p className="text-white cursor-default MovieOverview">{see || tv.overview.length < 400 ? tv.overview : `${tv.overview.substring(0, 400)}...`}</p>
                {ShowReadMore && (
                    <p className="text-sm">
